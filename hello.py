...
schedule.every(6).minutes.do(get_realtime_money_flow)    # 每隔1分钟执行一次任务
...
schedule.every(7).minutes.do(get_stock_sector_fund_flow_rank)    # 每隔1分钟执行一次任务

while True:
    if time_cmp(time.localtime(), time.strptime("09:30:00", "%H:%M:%S")) > 0 | time_cmp(time.localtime(), time.strptime("11:30:00", "%H:%M:%S")) < 0:
        isWitch = True
    elif time_cmp(time.localtime(), time.strptime("13:00:00", "%H:%M:%S")) > 0 | time_cmp(time.localtime(), time.strptime("15:00:00", "%H:%M:%S")) < 0:
        isWitch = True
    else:
        isWitch = False
